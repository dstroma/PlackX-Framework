...
sub do_tests {
  ...
  # MD5
  {
    ...
    foreach my $key (keys %known) {
      is(
        md5_ubase64($key) => $known{$key},
        'url-encoded MD5 is correct'
      );

      for my $len (1..16) {
        is(
          md5_ushort($key,$len) => substr($known{$key},0,$len),
    ...
    foreach my $key (keys %known_incorrect) {
      isnt(
        md5_ubase64($key) => $known_incorrect{$key},
  ...
  # JSON and Base64
  {
    ...
    is(
      b64_to_u64(MIME::Base64::encode($bin_str)) => $encoded,
      'Base64 to Url-safe encoded correctly',
    );
    is(
      MIME::Base64::decode(u64_to_b64($encoded)) => $bin_str,
    ...
    is_deeply(
      decode_ju64(encode_ju64($json_data)) => $json_data,
      'JSON encode and decode'
    );

    # No illegal characters
    ok(
      (encode_ju64($json_data) =~ m/^[a-zA-Z0-9_\-]+$/),
  ...
  # Modules
  {
    ...
    require Plack::Util;
    ok(
      is_module_loaded('Plack::Util'),
      'Module is loaded'
    );

    ok(
      (not is_module_broken('Plack::Util')),
      'Module is not broken'
    );
    is(
      name_to_pm('Plack::Util') => 'Plack/Util.pm',
